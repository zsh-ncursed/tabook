...
describe('fetchFeed', () => {
  ...
  it('follows redirects manually', async () => {
    ...
    const calls: string[] = [];
    setFetchMock(
      vi.fn(async (url, _init?) => {
        calls.push(String(url));
        if (calls.length === 1) {
          return mockResponse('', {
            status: 301,
            headers: { location: 'https://m.gutenberg.org/ebooks.opds/' },
          });
        }
        return mockResponse(xml);
      }),
  ...
  it('throws on too many redirects', async () => {
    setFetchMock(
      vi.fn(async () => mockResponse('', { status: 301, headers: { location: 'https://x/loop' } })),
    );
    ...
    setFetchMock(
      vi.fn(async (url, init?) => {
        ...
        if (calls.length === 1) {
          return mockResponse('', {
            status: 302,
            headers: { location: 'https://cdn.example.org/feed.xml' },
          });
        }
        return mockResponse(xml);
      }),
