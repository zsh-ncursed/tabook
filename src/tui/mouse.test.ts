...
describe('parseMouseChunk', () => {
  it('parses an SGR left-button press', () => {
    expect(parseAll('\x1b[<0;5;10M')).toEqual([{ x: 5, y: 10, button: 'left', press: true }]);
  });

  it('parses an SGR left-button release', () => {
    expect(parseAll('\x1b[<0;5;10m')).toEqual([{ x: 5, y: 10, button: 'left', press: false }]);
  });

  it('distinguishes middle and right buttons', () => {
    expect(parseAll('\x1b[<1;1;1M')).toEqual([{ x: 1, y: 1, button: 'middle', press: true }]);
    expect(parseAll('\x1b[<2;1;1M')).toEqual([{ x: 1, y: 1, button: 'right', press: true }]);
  });

  it('parses multiple events in one chunk', () => {
    const clicks = parseAll('\x1b[<0;1;1M\x1b[<0;1;1m\x1b[<0;2;1M');
    expect(clicks).toHaveLength(3);
    expect(clicks[1]).toMatchObject({ x: 1, y: 1, press: false });
    expect(clicks[2]).toMatchObject({ x: 2, y: 1, press: true });
  });
